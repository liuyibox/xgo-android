...
for TARGET in $TARGETS; do
  ...
  if ([ $XGOOS == "." ] || [[ $XGOOS == android* ]]); then
    ...
    if [ $XGOARCH == "." ] || [ $XGOARCH == "arm" ] || [ $XGOARCH == "aar" ]; then
      if [ "$GO_VERSION" -lt 150 ]; then
        ...
        $ANDROID_NDK_ROOT/build/tools/make-standalone-toolchain.sh --verbose --install-dir=/usr/$ANDROID_CHAIN_ARM --toolchain=$ANDROID_CHAIN_ARM --arch=arm #> /dev/null 2>&1
        ...
        if [ $XGOARCH == "." ] || [ $XGOARCH == "arm" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=arm-linux-androideabi-gcc CXX=arm-linux-androideabi-g++ GOOS=android GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=arm-linux-androideabi-gcc CXX=arm-linux-androideabi-g++ GOOS=android GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X $MOD "${T[@]}" --ldflags="$V $EXT_LDPIE $EXT_LDAMD $LD" $R $BM -o "/build/$NAME-android-$PLATFORM-arm`extension android`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "aar" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=arm-linux-androideabi-gcc CXX=arm-linux-androideabi-g++ GOOS=android GOARCH=arm GOARM=7 CGO_ENABLED=1 go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=arm-linux-androideabi-gcc CXX=arm-linux-androideabi-g++ GOOS=android GOARCH=arm GOARM=7 CGO_ENABLED=1 go build $V $X $MOD "${T[@]}" --ldflags="$V $EXT_LDAMD $LD" $BM -o "/build-android-aar/$NAME-android-$PLATFORM-arm.so" $PACK_RELPATH
        fi
      fi
    fi
    if [ "$GO_VERSION" -lt 160 ]; then
      ...
      if [ "$PLATFORM" -ge 9 ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "386" ] || [ $XGOARCH == "aar" ]); then
        ...
        $ANDROID_NDK_ROOT/build/tools/make-standalone-toolchain.sh --verbose --install-dir=/usr/$ANDROID_CHAIN_386 --toolchain=$ANDROID_CHAIN_386 --arch=x86 #> /dev/null 2>&1
        ...
        if [ $XGOARCH == "." ] || [ $XGOARCH == "386" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=i686-linux-android-gcc CXX=i686-linux-android-g++ GOOS=android GOARCH=386 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=i686-linux-android-gcc CXX=i686-linux-android-g++ GOOS=android GOARCH=386 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X $MOD "${T[@]}" --ldflags="$V $EXT_LDPIE $LD" $BM -o "/build/$NAME-android-$PLATFORM-386`extension android`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "aar" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=i686-linux-android-gcc CXX=i686-linux-android-g++ GOOS=android GOARCH=386 CGO_ENABLED=1 go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=i686-linux-android-gcc CXX=i686-linux-android-g++ GOOS=android GOARCH=386 CGO_ENABLED=1 go build $V $X $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build-android-aar/$NAME-android-$PLATFORM-386.so" $PACK_RELPATH
        fi
      fi
      if [ "$PLATFORM" -ge 21 ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "arm64" ] || [ $XGOARCH == "aar" ]); then
        echo "Assembling toolchain for android-$PLATFORM/arm64..."
        $ANDROID_NDK_ROOT/build/tools/make-standalone-toolchain.sh --verbose --install-dir=/usr/$ANDROID_CHAIN_ARM64 --toolchain=$ANDROID_CHAIN_ARM64 --arch=arm64 #> /dev/null 2>&1
        ...
        if [ $XGOARCH == "." ] || [ $XGOARCH == "arm64" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=aarch64-linux-android-gcc CXX=aarch64-linux-android-g++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=aarch64-linux-android-gcc CXX=aarch64-linux-android-g++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X $MOD "${T[@]}" --ldflags="$V $EXT_LDPIE $LD" $BM -o "/build/$NAME-android-$PLATFORM-arm64`extension android`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "aar" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=aarch64-linux-android-gcc CXX=aarch64-linux-android-g++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=aarch64-linux-android-gcc CXX=aarch64-linux-android-g++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 go build $V $X $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build-android-aar/$NAME-android-$PLATFORM-arm64.so" $PACK_RELPATH
        fi
      fi
      if [ "$PLATFORM" -ge 21 ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "amd64" ] || [ $XGOARCH == "aar" ]); then
        echo "Assembling toolchain for android-$PLATFORM/amd64..."
        $ANDROID_NDK_ROOT/build/tools/make-standalone-toolchain.sh --verbose --install-dir=/usr/$ANDROID_CHAIN_AMD64 --toolchain=$ANDROID_CHAIN_AMD64 --arch=x86_64 #> /dev/null 2>&1

        echo "Bootstrapping android-$PLATFORM/amd64..."
        CC=x86_64-linux-android-gcc GOOS=android GOARCH=amd64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go install std

        echo "Compiling for android-$PLATFORM/amd64..."
        CC=x86_64-linux-android-gcc CXX=x86_64-linux-android-g++ HOST=x86_64-linux-android PREFIX=/usr/$ANDROID_CHAIN_AMD64/x86_64-linux-android $BUILD_DEPS /deps ${DEPS_ARGS[@]}
        export PKG_CONFIG_PATH=/usr/$ANDROID_CHAIN_AMD64/x86_64-linux-android/lib/pkgconfig

        if [ $XGOARCH == "." ] || [ $XGOARCH == "amd64" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=x86_64-linux-android-gcc CXX=x86_64-linux-android-g++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=x86_64-linux-android-gcc CXX=x86_64-linux-android-g++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X $MOD "${T[@]}" --ldflags="$V $EXT_LDPIE $LD" $BM -o "/build/$NAME-android-$PLATFORM-amd64`extension android`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "aar" ]; then
          if [[ "$USEMODULES" == false ]]; then
            CC=x86_64-linux-android-gcc CXX=x86_64-linux-android-g++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
          fi
          CC=x86_64-linux-android-gcc CXX=x86_64-linux-android-g++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 go build $V $X $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build-android-aar/$NAME-android-$PLATFORM-amd64.so" $PACK_RELPATH
        fi
      fi
    fi
    ...
    if [ $XGOARCH == "." ] || [ $XGOARCH == "aar" ]; then
      ...
      package=`go list $PACK_RELPATH | tr '-' '_'`
      ...
      if [ "$header" == "" ]; then
        ...
        for lib in `find /build-android-aar | grep '\.so$'`; do
          ...
          if [[ "$lib" = *-amd64.so ]]; then cc=x86_64-linux-android-gcc; abi="x86_64"; fi
    ...
    rm -rf /usr/$ANDROID_CHAIN_ARM /usr/$ANDROID_CHAIN_ARM64 /usr/$ANDROID_CHAIN_AMD64 /usr/$ANDROID_CHAIN_386
  fi
  ...
  if [ $XGOOS == "." ] || [[ $XGOOS == ios* ]]; then
    ...
    if [ "$PLATFORM" == "" ] || [ "$PLATFORM" == "." ] || [ "$PLATFORM" == "ios" ]; then
      PLATFORM=7.0 # first iPad and upwards
    fi
    ...
    if [ "$GO_VERSION" -lt 150 ]; then
      ...
      cp -r $OSX_NDK_CROSS/SDK/$OSX_SDK/usr/include/* $IOS_NDK_ARM64/SDK/$IOS_SDK_VERSION.sdk/usr/include/
      mkdir -p $IOS_NDK_ARM64/SDK/$IOS_SDK_VERSION.sdk/usr/include/IOKit
      cp -r $OSX_NDK_CROSS/SDK/$OSX_SDK/System/Library/Frameworks/IOKit.framework/Versions/A/Headers/* $IOS_NDK_ARM64/SDK/$IOS_SDK_VERSION.sdk/usr/include/IOKit/
      ...
      if [ -d "$IOS_NDK_ARM_7" ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "arm-7" ] || [ $XGOARCH == "framework" ]); then
        ...
        if [[ "$USEMODULES" == false ]]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm GOARM=7 CGO_ENABLED=1 go get $V $X "${IOSTAGS[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "arm-7" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm GOARM=7 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$LDSTRIP $V $LD" $BM -o "/build/$NAME-ios-$PLATFORM-armv7`extension darwin`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "framework" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm GOARM=7 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$V $LD" --buildmode=c-archive -o "/build-ios-fw/$NAME-ios-$PLATFORM-armv7.a" $PACK_RELPATH
        fi
      ...
      if [ -d "$IOS_NDK_ARM64" ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "arm64" ] || [ $XGOARCH == "framework" ]); then
        ...
        CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ HOST=arm-apple-darwin11 PREFIX=/usr/local $BUILD_DEPS /deps ${DEPS_ARGS[@]}
        if [[ "$USEMODULES" == false ]]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 go get $V $X "${IOSTAGS[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "arm64" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$LDSTRIP $V $LD" $BM -o "/build/$NAME-ios-$PLATFORM-arm64`extension darwin`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "framework" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$V $LD" --buildmode=c-archive -o "/build-ios-fw/$NAME-ios-$PLATFORM-arm64.a" $PACK_RELPATH
        fi
      ...
      if [ -d "$IOS_SIM_NDK_AMD64" ] && ([ $XGOARCH == "." ] || [ $XGOARCH == "amd64" ] || [ $XGOARCH == "framework" ]); then
        # TODO not really supported atm. $IOS_SIM_NDK_AMD64 is not even a directory
        ...
        CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ HOST=arm-apple-darwin11 PREFIX=/usr/local $BUILD_DEPS /deps ${DEPS_ARGS[@]}
        if [[ "$USEMODULES" == false ]]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 go get $V $X "${IOSTAGS[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "amd64" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$LDSTRIP $V $LD" $BM -o "/build/$NAME-ios-$PLATFORM-x86_64`extension darwin`" $PACK_RELPATH
        fi
        if [ $XGOARCH == "." ] || [ $XGOARCH == "framework" ]; then
          CC=arm-apple-darwin11-clang CXX=arm-apple-darwin11-clang++ GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 go build $V $X $MOD "${IOSTAGS[@]}" --ldflags="$V $LD" --buildmode=c-archive -o "/build-ios-fw/$NAME-ios-$PLATFORM-x86_64.a" $PACK_RELPATH
        fi
